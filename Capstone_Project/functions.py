""" A set of data parsing functions

A collection of Functions to import the data and tranfrom them in
the desired format
...
def convert_df_to_ts (dataframe, countries):
    """Creates aggregated statistics at a daily level for each country passed.

    Aggregates the statistics as defined in the convert_to_ts function for
    an entire array of specified countries.

    Merges the result in one single distinct dataframe.
    ...
    for country in countries:
        ...
        size = convert_to_ts(dataframe, country= country).shape[0]

        country_cat = pd.DataFrame({ "country": np.repeat(country,size).
                                  flatten()})

        df_ts = pd.concat([df_ts,
                           pd.concat([convert_to_ts(
...
def engineer_features(df,training=True):
    ...
    ## extract the days present in the series
    for d,day in enumerate(dates):
        ...
        for num in previous:
            ...
            mask = np.in1d(dates, np.arange(prev,current,dtype='datetime64[D]')) # bool
                                                                                 # vector
                                                                                 # that
                                                                                 # checks
                                                                                 # if
    ...
    return(X,y,dates) ## features, target, dates


def engineer_features_by_country (dataframe, training, countries):
    """Creates aggregated statistics at a daily level for each country passed.
    ...
    for country in countries:
        ...
        bol = np.array(dataframe[["country"]] == country)

        eng_features = engineer_features(dataframe[bol], training)

        size = eng_features[0].shape[0]

        country_cat = pd.DataFrame({ "country": np.repeat(country,size).
                                  flatten()})

        df_ts = pd.concat([df_ts,
                           pd.concat([eng_features[0],
                                      pd.Series(eng_features[1], name = "y_revenue"),
                                      pd.Series(eng_features[2], name = "date"),
                                      country_cat],
                                     axis = 1)],
                          axis = 0)
